...
```{r}
isd_stats <- isds |> group_by(district) |> 
  summarize(total_adjustments = sum(final_financial_adj, na.rm = TRUE),
            count_rows = n(),
            avg_financial_adj = mean(final_financial_adj, na.rm = TRUE) |> round_half_up(),
            most_neg_financial_adj = min(final_financial_adj, na.rm = TRUE),
            most_pos_financial_adj = max(final_financial_adj, na.rm = TRUE),
            median_financial_adj = median(final_financial_adj, na.rm = TRUE),
            largest_financial_adj = final_financial_adj[which.max(abs(final_financial_adj))]) |> 
  arrange(total_adjustments)
...
```{r}
isd_cisd_msd <- audits |> filter(str_detect(district, "ISD|MSD|CONSOLIDATES ISD"))

charter_other <- audits |> filter(!str_detect(district,"ISD|MSD|CONSOLIDATED ISD"))
...
isd_cisd_msd_stats <- isd_cisd_msd |> group_by(district) |> 
  summarize(total_adjustments = sum(final_financial_adj, na.rm = TRUE),
            count_rows = n(),
            avg_financial_adj = mean(final_financial_adj, na.rm = TRUE) |> round_half_up(),
            most_neg_financial_adj = min(final_financial_adj, na.rm = TRUE),
...
Now for the charter and other schools.

```{r}
charter_other |> group_by(district) |> 
  summarize(total_adjustments = sum(final_financial_adj, na.rm = TRUE),
            count_rows = n(),
            avg_financial_adj = mean(final_financial_adj, na.rm = TRUE) |> round_half_up(),
            most_neg_financial_adj = min(final_financial_adj, na.rm = TRUE),
...
```{r}
isds |> filter(district == "PROGRESO ISD")
```

```{r}
isds |> filter(district == "SOUTH TEXAS ISD")
```
